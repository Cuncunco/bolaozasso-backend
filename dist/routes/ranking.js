...
function pointsForGuess(g1, g2, r1, r2) {
...
export async function rankingRoutes(fastify) {
    ...
    fastify.put("/pools/:poolId/games/:gameId/result", { preHandler: [requireAuth] }, async (request, reply) => {
    ...
    fastify.delete("/pools/:poolId/games/:gameId/result", { preHandler: [requireAuth] }, async (request, reply) => {
        const paramsSchema = z.object({
            poolId: z.string(),
            gameId: z.string(),
        });
    ...
    fastify.get("/pools/:poolId/ranking", async (request, reply) => {
        ...
        for (const g of guesses) {
            ...
            scoreByParticipant.set(g.participantId, current +
                pointsForGuess(g.firstTeamPoints, g.secondTeamPoints, result.firstTeamPoints, result.secondTeamPoints));
        }
